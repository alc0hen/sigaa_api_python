...
async def create_client(payload: NewClientRequest, _: str=Depends(verify_admin)):
    ...
    private_hex = private_key.private_bytes(encoding=serialization.Encoding.Raw, format=serialization.PrivateFormat.Raw, encryption_algorithm=serialization.NoEncryption()).hex()
    public_hex = public_key.public_bytes(encoding=serialization.Encoding.Raw, format=serialization.PublicFormat.Raw).hex()
    auth.register_client(public_hex, payload.name)
    return {'public_key': public_hex, 'private_key': private_hex, 'name': payload.name}
...
async def get_logs(_: str=Depends(verify_admin)):
    ...
    if os.path.exists(LOG_FILE):
        try:
            ...
            pass
    logs.reverse()
    return {'active_requests': _active_requests, 'logs': logs}
...
class SignedRequestMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            auth.verify_request(public_key_hex=client_public_key, signature_hex=request.headers.get('X-Signature'), timestamp=request.headers.get('X-Timestamp'), method=request.method, path=signed_path, body=body)
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            log_entry = {'timestamp': int(time.time()), 'client': client_key, 'method': request.method, 'path': path, 'status': status_code, 'duration_ms': duration_ms}
            try:
...
async def create_session(client_public_key: str, payload: LoginRequest, request: Request):
    ...
    async with _sessions_lock:
        _sessions[session_id] = {'sigaa': sigaa, 'account': account, 'client_public_key': client_public_key, 'last_used': time.time(), 'credentials': {'username': payload.username, 'password': payload.password, 'url': payload.url, 'inst_type': institution}, 'courses': {}, 'enrollment': {}}
...
async def list_courses(client_public_key: str, session_id: str, bond_id: str, request: Request):
    ...
    return {'courses': [{'id': i, 'title': c.title, 'schedule_code': c.schedule_code} for i, c in enumerate(courses)]}

@app.get('/api/v1/{client_public_key}/sessions/{session_id}/bonds/{bond_id}/courses/{course_id}/details')
async def course_details(client_public_key: str, session_id: str, bond_id: str, course_id: int, request: Request):
    session = await _get_owned_session(session_id, request.state.client_public_key)
    bond = _student_bond(session, bond_id)
    ...
    return {'id': course_id, 'title': course.title, 'schedule_code': course.schedule_code, 'grades': grades, 'frequency': frequency, 'professor': professor}

@app.post('/api/v1/{client_public_key}/sessions/{session_id}/bonds/{bond_id}/history')
async def bond_history(client_public_key: str, session_id: str, bond_id: str, payload: HistoryRequest, request: Request):
    session = await _get_owned_session(session_id, request.state.client_public_key)
    bond = _student_bond(session, bond_id)
    await _assert_sigaa_alive(session)
    credentials = session['credentials'] if payload.parallel else None
    async with _scraper_errors():
        history = await bond.get_history(cached_history=payload.cached_history, credentials=credentials)
    return {'history': history}

@app.get('/api/v1/{client_public_key}/sessions/{session_id}/bonds/{bond_id}/enrollment')
async def enrollment_disciplines(client_public_key: str, session_id: str, bond_id: str, request: Request):
    session = await _get_owned_session(session_id, request.state.client_public_key)
    bond = _student_bond(session, bond_id)
    await _assert_sigaa_alive(session)
    async with _scraper_errors():
        result = await bond.get_enrollment_disciplines()
    session.setdefault('enrollment', {})[bond_id] = {'view_state': result.get('view_state'), 'action_url': result.get('action_url')}
...
async def enrollment_selection(client_public_key: str, session_id: str, bond_id: str, payload: EnrollmentSelectionRequest, request: Request):
    session = await _get_owned_session(session_id, request.state.client_public_key)
    bond = _student_bond(session, bond_id)
    ...
    async with _scraper_errors():
        page = await bond.submit_enrollment(payload.selected_class_ids, view_state, action_url=state.get('action_url'))
...
async def enrollment_confirm(client_public_key: str, session_id: str, bond_id: str, payload: EnrollmentConfirmRequest, request: Request):
    session = await _get_owned_session(session_id, request.state.client_public_key)
    bond = _student_bond(session, bond_id)
    state = session.setdefault('enrollment', {}).get(bond_id, {})
    ...
    async with _scraper_errors():
        ...
        final_page = await bond.confirm_enrollment(payload.password, password_page.view_state, password_page.body)
